...
$ReplacePattern            = '[^a-zA-Z0-9]'#This pattern is used, to convert the $DynamicsAXApiId in to useable format for a Storage accounts container. If blank, default is '[^a-zA-Z0-9]' 
$AADSecretKeyBackup        = "C:\Users\magra\Google Drive\Arbejde\CORO\D365O\ExFlow installation\TEST12" #Parameter used to speficy where the AADsecret should be saved. Also used when upgrading! Example: c:\temp or c:\backup How not to: c:\temp\
...
Invoke-Command -ScriptBlock ([scriptblock]::Create($scriptPath)) -ArgumentList $Location,$Security_Admins,$DynamicsAXApiId,$ExFlowUserSecret,$Prefix,$PackageVersion,$TenantGuid,$WebAppSubscriptionGuid,$TenantnameSpecific,$ReplacePattern,$AADSecretKeyBackup